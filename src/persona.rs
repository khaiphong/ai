...
struct Prajna<T>(T) { 
	type item;
	data String; // current rating via evaluation of training programs and detoxification
...
// In graph data structures, multiple edges (friends / breakthroughs as outcome of Prajna
 node) might point to the same node, and that node is conceptually owned by all of the edges 
 that point to it. The Rc<T> type keeps track of the number of references to a value to 
 determine whether or not the value is still in use. Similarly, for a rated positive f there 
 are many outcomes of smart pointers pointing to different traits
